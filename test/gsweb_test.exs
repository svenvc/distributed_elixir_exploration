defmodule GswebTest do
  ...
  test "key-value genserver" do
    ...
    assert(Req.post!("http://localhost:4000/process/kv1/call", json: ["error"]).body == "not_implemented")
  end
  ...
  test "pub-sub genserver" do
    Process.spawn(fn ->
      Process.sleep(100)
      assert(
        Req.post!("http://localhost:4000/process/ps1/call", json: ["broadcast", "topic1", ["msg", 101]]).body
      )
    end, [])
